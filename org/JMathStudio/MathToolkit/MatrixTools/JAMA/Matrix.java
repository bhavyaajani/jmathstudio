...
class Matrix implements Cloneable, java.io.Serializable {
	...
	public Cell getMatrixCopyAsCell(){
		Cell res = new Cell(this.m,this.n);
		
		for(int i=0;i<m;i++)
		{
			for(int j=0;j<n;j++)
			{
				res.setElement((float)this.A[i][j], i, j);
			}
		}
		return res;
	}
	...
	public Cell solveAdapter(Matrix B) {
		return (m == n ? (new LUDecomposition(this)).solve(B).getMatrixCopyAsCell()
				: (new QRDecomposition(this)).solve(B).getMatrixCopyAsCell());
	}
	...
	public Cell solveTransposeAdapter(Matrix B) {
		return transpose().solve(B.transpose()).getMatrixCopyAsCell();
	}
	...
	public Cell inverseAdapter() {
		return solve(identity(m, m)).getMatrixCopyAsCell();
	}
