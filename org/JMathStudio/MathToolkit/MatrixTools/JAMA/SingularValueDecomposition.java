...
class SingularValueDecomposition implements java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3753476927933634928L;
	...
	public Cell getUAdapter() {
		return new Matrix(U, m, Math.min(m + 1, n)).getMatrixCopyAsCell();
	}
	...
	public Cell getVAdapter() {
		return new Matrix(V, n, n).getMatrixCopyAsCell();
	}
	...
	public Cell getSAdapter() {
		Cell X = new Cell(n, n);
		float[][] S = X.accessCellBuffer();
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				S[i][j] = 0.0f;
			}
			S[i][i] = (float)this.s[i];
		}
		return X;
	}
